...
export async function GET(request: Request) {
  ...
  try {
    ...
    rememberBookingSnapshot({ business, services });
    ...
    logBookingApiError("event_types", err);
    if (!isDbConnectivityError(err)) {
      return NextResponse.json(
        { detail: "Unable to load bookable spaces." },
        { status: 503 }
      );
    }
    ...
    const services = snapshotServices(businessId, active).slice(0, count);
